    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-4">
      <div className="max-w-md w-full">
        <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-8 text-center relative">
          <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl font-bold transition">&times;</button>
...
const ForgotPasswordPage = ({ onBack, onClose, onSubmit }) => {
  ...
  const handleSubmit = async () => {
    if (!email.trim()) { setStatus({ type: 'error', message: 'Please enter your email address.' }); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setStatus({ type: 'error', message: 'Please enter a valid email address.' }); return; }
    setLoading(true);
    setStatus({ type: '', message: '' });
    try {
      const response = await fetch('https://your-api-domain.com/api/forgot-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });
      if (response.ok) {
        setStatus({ type: 'success', message: 'OTP code has been sent to your email!' });
        setTimeout(() => onSubmit(), 1000);
      } else {
        setStatus({ type: 'error', message: 'Email not found. Please try again.' });
      }
    } catch {
      setStatus({ type: 'success', message: 'OTP code has been sent to your email!' });
      setTimeout(() => onSubmit(), 1000);
        ...
        <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-8 text-center relative">
          ...
          <div className="flex justify-center mb-6">
            <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center">
              <img src="https://cdn-icons-png.flaticon.com/512/6195/6195700.png" alt="lock" className="w-12 h-12" />
            </div>
          </div>
          ...
          <div className="relative mb-5 text-left">
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1.5 pl-1">Email Address</label>
            <div className="relative">
              <img src="https://cdn-icons-png.flaticon.com/512/732/732200.png" alt="email" className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 opacity-50" />
              <input
                type="email"
...
function Register({ onClose, onRegisterSuccess, onForgotPassword }) {
  ...
  const handleRegister = async () => {
    if (!form.name || !form.email || !form.password || !form.confirm) {
      setStatus({ type: 'error', message: 'Please fill in all fields.' }); return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) {
      setStatus({ type: 'error', message: 'Please enter a valid email.' }); return;
    }
    if (form.password.length < 6) {
      setStatus({ type: 'error', message: 'Password must be at least 6 characters.' }); return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setStatus({ type: 'success', message: 'Account created! Sending OTP...' });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-4">
      ...
      <div className="bg-white w-full max-w-sm sm:max-w-md p-5 sm:p-6 rounded-2xl shadow-xl relative">
        ...
        <div className="flex flex-col items-center mb-4">
          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRmfgEsISgcMna9mdI-t_XY7o-WkAI0ctitvg&s" alt="" className="w-12 mb-1" />
        ...
        <div className="space-y-2.5">
          {/* Full Name */}
          <div className="relative">
            <img src="https://cdn-icons-png.flaticon.com/512/1077/1077114.png" alt="user" className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 opacity-60" />
          ...
          {/* Email */}
          <div className="relative">
            <img src="https://cdn-icons-png.flaticon.com/512/732/732200.png" alt="email" className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 opacity-60" />
          ...
          {/* Password */}
          <div className="relative">
            <img src="https://cdn-icons-png.flaticon.com/512/3064/3064155.png" alt="lock" className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 opacity-60" />
          ...
          <div className="flex justify-end">
            <button onClick={onForgotPassword} className="text-xs text-blue-500 hover:underline font-medium">
              Forgot Password?
            </button>
          </div>
          ...
          {/* Status */}
          {status.message && (
            <div className={`text-xs font-semibold text-center ${status.type === 'error' ? 'text-red-500' : 'text-green-600'}`}>
              {status.message}
            </div>
          )}
          ...
          <div className="flex gap-2 items-center">
            ...
            <label htmlFor="keepLogin" className="text-blue-500 text-sm cursor-pointer">Keep me login</label>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-1 pt-1">
            ...
            <span className="text-blue-500 text-sm font-semibold cursor-pointer hover:underline">Login here</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <nav className="fixed top-6 left-0 w-full z-50 px-6 lg:px-16">
        <div className="max-w-7xl mx-auto bg-white/90 backdrop-blur-md rounded-full shadow-lg px-4 py-2 flex items-center justify-between border border-white/20">
          ...
          <ul className="hidden lg:flex items-center gap-6 text-[11px] font-bold uppercase tracking-widest">
            ...
            <li onClick={() => setModal("register")} className="text-[#0D47A1] cursor-pointer hover:text-blue-600 transition normal-case">
              Register
            </li>
          </ul>
